...
export default async function ProviderUpdateList({
  providerJson,
}: {
  ...
  return (
    <>
      <h2 className="text-2xl font-medium mb-6 text-black text-center font-robotoMono">
        We have up to {providerJsonReady.data.length} providers to be updated.
      </h2>
      <div className="flex flex-wrap justify-center gap-8 px-4">
        {providerJsonReady.data.map((providerItem: ProviderItem, index) => (
          <Link
            href={`/manage/provider/update/${providerItem.id}`}
            key={providerItem.id}
          ...
          >
            <div className="border border-gray-300 rounded-lg overflow-hidden shadow-lg bg-white relative group hover:scale-105 transition-all ease-in-out">
              ...
              <div className="absolute inset-0 flex justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-xl font-bold text-white font-robotoMono">Update</span>
              </div>
            </div>
          </Link>
        ))}
